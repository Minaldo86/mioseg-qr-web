...
const HOME_RELEASE_COPY: Record<PublicLocale, {
  ...
  de: {
    ...
    finalExplore: "Live-Beispiele ansehen",
...
export default async function Home({ params }: Props) {
  ...
  const masterCopy = publicLanguage === "de" ? {
    learnMore:"Live-Beispiel ansehen",
    ...
    <main className={`${styles.page} landingBPage`} style={{ background: "#06101f", minHeight: "100vh" }}>
      ...
      <footer className="landingBFooter">
        ...
        <div className="landingBFooterLinks">
          <Link href={`/${locale}#${useMasterLayout ? "qrx-explained" : "features"}`}>{heroCopy.navFeatures}</Link>
          <Link href={`/${locale}/explore`}>{heroCopy.navExplore}</Link>
          <Link href={`/${locale}#${useMasterLayout ? "live-examples" : "usecases"}`}>{heroCopy.navUseCases}</Link>
